...
export class MavenProject implements MavenProjectData {
   ...
   */
  getAll(): MavenModule[] {
    return Object.values(this.modules).filter((v, i, arr) => arr.indexOf(v) === i);
  }
    ...
    while (dir !== rootPath) {
      ...
      try {
        ...
        if (pom.project && pom.project.modules) {
          const rootModule = (await import('../module')).MavenModule.fromData({
            absolutePath: dir,
            relativePath: '.',
          ...
          if (Array.isArray(moduleNames)) {
            const children = await Promise.all(
              moduleNames.map(async (modName: string) => {
                ...
                const childPom: PomXml = parsePomSync(path.join(absPath, 'pom.xml'));
                const childParentPom: PomXml = parsePomSync(path.join(absPath, '..', 'pom.xml'));
                return (await import('../module')).MavenModule.fromData({
                  absolutePath: absPath,
                  relativePath: '.',
                  name: path.basename(absPath),
                  artifactId: childPom.project?.artifactId ?? path.basename(absPath),
            ...
            children.forEach((mod: unknown) => {
              if (mod) childModules.push(mod as MavenModule);
            });
          }
