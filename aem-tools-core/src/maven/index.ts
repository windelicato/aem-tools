...
export async function runMaven(config: ResolvedConfig, opts: MavenOpts = {}) {
  ...
  if (!project) {
    console.error('Could not find a Maven project (no root pom.xml with <modules> found).');
    return;
  }
  ...
  if (opts.module) {
    ...
    target =
      project.getAll().reduce((best: MavenModule | undefined, mod: MavenModule) => {
  ...
  const profilePart = opts.profile ? `-P${opts.profile}` : target.profiles[0] ? `-P${target.profiles[0]}` : '';
  const skipTestsPart = opts.skipTests || config.maven.skipTests ? '-DskipTests' : '';
