 ...
 */
export async function parseMavenModule(dir: string, isRoot = false): Promise<import('./index').MavenModule | null> {
  ...
  try {
    ...
    const { resolveProfiles } = await import('./resolver');
    const { MavenModule } = await import('./index');
