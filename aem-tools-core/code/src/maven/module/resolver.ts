 ...
 */
export function resolveProfiles(pom: PomXml, parentPom?: PomXml, isRoot = false): string[] {
  const profiles: string[] = [];

  // Normalize profiles
  const moduleProfiles = toArray<PomProfile>(pom.project?.profiles?.profile);
  const parentProfiles = toArray<PomProfile>(parentPom?.project?.profiles?.profile);
  ...
  if (
    ...
    pluginsArr.some(
      (p) => p?.artifactId === 'content-package-maven-plugin' || p?.artifactId === 'filevault-package-maven-plugin',
    )
  ) {
  ...
  const hasAutoInstallBundle = parentProfiles.some((prof) => prof?.id === 'autoInstallBundle');
