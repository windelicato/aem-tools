...
function resolveField<T>(field: FieldResolver<T>, fileVal: unknown): T {
  const fromEnv = typeof process !== 'undefined' ? process.env[field.env] : undefined;
  if (fromEnv !== undefined) {
    try {
      if (typeof field.default === 'boolean') return (fromEnv === 'true') as unknown as T;
      if (typeof field.default === 'number') return Number(fromEnv) as unknown as T;
...
function resolveSection<S extends Record<string, FieldResolver<unknown>>>(
  ...
  return Object.fromEntries(
    Object.entries(schema).map(([key, resolver]) => {
      if (!isFieldResolver(resolver)) throw new Error(`Invalid schema for key: ${key}`);
 ...
 */
function getSection<T extends object>(fileData: unknown, section: string): Partial<T> {
  if (
...
export function loadConfigSync(configPath = '.aemrc.json') {
  const configFilePath = typeof process !== 'undefined' ? path.resolve(process.cwd(), configPath) : '';
  const fileData: unknown =
    configFilePath && fs.existsSync(configFilePath) ? JSON.parse(fs.readFileSync(configFilePath, 'utf-8')) : {};
